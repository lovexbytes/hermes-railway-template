...
# Saeed: persistent ElevenLabs keys (survives redeploy — keys live in /data volume, not image)
if [ -f "/data/.hermes/scripts/load_elevenlabs_keys.sh" ]; then
    # shellcheck source=/dev/null
    . "/data/.hermes/scripts/load_elevenlabs_keys.sh"
fi

# Saeed: rebuild /tmp artifacts that live on ephemeral storage (WebUI, tunnel, etc.)
if [ -f "/data/.hermes/scripts/boot-installs.sh" ]; then
    /data/.hermes/scripts/boot-installs.sh || true
fi
...
for key in \
  ...
  FIRECRAWL_API_KEY NOUS_API_KEY BROWSERBASE_API_KEY BROWSERBASE_PROJECT_ID BROWSERBASE_PROXIES BROWSERBASE_ADVANCED_STEALTH BROWSER_SESSION_TIMEOUT BROWSER_INACTIVITY_TIMEOUT FAL_KEY ELEVENLABS_API_KEY ELEVENLABS_API_KEY_2 ELEVENLABS_API_KEY_3 ELEVENLABS_API_KEY_4 ELEVENLABS_API_KEY_5 VOICE_TOOLS_OPENAI_KEY \
