...
EVOLINK_BASE_URL="${EVOLINK_BASE_URL:-https://direct.evolink.ai/v1}"
...
configure_evolink_provider() {
  if [[ -z "${EVOLINK_API_KEY:-}" ]]; then
    return 0
  fi

  export OPENAI_BASE_URL="${OPENAI_BASE_URL:-${EVOLINK_BASE_URL}}"
  export OPENAI_API_KEY="${OPENAI_API_KEY:-${EVOLINK_API_KEY}}"
  export HERMES_INFERENCE_PROVIDER="${HERMES_INFERENCE_PROVIDER:-openai}"
...
if ! has_valid_provider_config; then
  echo "[bootstrap] ERROR: Configure a provider: EVOLINK_API_KEY, OPENROUTER_API_KEY, OPENAI_BASE_URL+OPENAI_API_KEY, or ANTHROPIC_API_KEY." >&2
  exit 1
fi
...
for key in \
  EVOLINK_API_KEY EVOLINK_BASE_URL OPENROUTER_API_KEY OPENAI_API_KEY OPENAI_BASE_URL ANTHROPIC_API_KEY LLM_MODEL HERMES_INFERENCE_PROVIDER HERMES_PORTAL_BASE_URL NOUS_INFERENCE_BASE_URL HERMES_NOUS_MIN_KEY_TTL_SECONDS HERMES_DUMP_REQUESTS \
